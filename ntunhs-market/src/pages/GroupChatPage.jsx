  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '32px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        <div style={{ width: 42, height: 42, background: 'linear-gradient(135deg,#0ea5e9,#0284c7)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
          <UsersIcon />
        </div>
        <div>
      ...
      <div style={{ background: '#fff', borderRadius: 18, boxShadow: '0 2px 14px rgba(0,0,0,.07)', overflow: 'hidden', display: 'flex', flexDirection: 'column', height: 520 }}>
            ...
            <div key={m.id} style={{ display: 'flex', justifyContent: m.isMe ? 'flex-end' : 'flex-start', gap: 8, alignItems: 'flex-end' }}>
              ...
              <div style={{ maxWidth: '60%' }}>
                ...
                <div style={{
                  ...
                  background: m.isMe ? 'linear-gradient(135deg,#0ea5e9,#0284c7)' : '#f1f5f9',
                  color: m.isMe ? '#fff' : '#1e293b',
        ...
        <form onSubmit={send} style={{ padding: '14px 20px', borderTop: '1px solid #f1f5f9', display: 'flex', gap: 10 }}>
          <input
            value={msg} onChange={e => setMsg(e.target.value)} placeholder="輸入訊息，與全校同學交流..."
